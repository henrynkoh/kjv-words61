...
export default function LandingPage({ topics }: Props) {
  ...
  const navItems = [
    ...FIXED_NAV.slice(0, 2), // Home, About
    { id: "topics", label: "Topics" },
    ...topics.map((t) => ({ id: t.id, label: t.title })),
    ...FIXED_NAV.slice(2), // Features, Verse sample, Footer
  ];

  return (
    <div className="flex min-h-screen bg-background">
      {/* Left sidebar - scrollable nav with all sub-sections */}
      <aside
      ...
      <div className="flex-1 min-w-0">
        ...
        >
          ...
          <p className="text-foreground/70 mb-2 max-w-2xl">
            Click any topic to see all verses with KJV text and Korean summaries.
          </p>
        </section>

        {/* One sub-section per topic - all content visible */}
        {topics.map((topic, i) => (
          <section
            key={topic.id}
            id={topic.id}
          ...
          >
            ...
            >
              ...
              {/* Preview first 2 verses so content is visible on landing */}
        ...
        >
          <div className="max-w-3xl text-sm text-foreground/70">
            <p>
              &copy; {new Date().getFullYear()} KJV Words. Built for spreading
              words of truth.
            </p>
      ...
      {/* Bottom-right: GitHub link - fixed, clickable */}
      <a
        href={GITHUB_URL}
        target="_blank"
